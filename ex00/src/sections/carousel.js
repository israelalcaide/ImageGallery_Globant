...
function create_card(p) {
  ...
  card.innerHTML =
    ...
    '<div class="card-info">' +
      '<h3 class="card-title">' + p.alt + "</h3>" +
      '<p class="card-category">por ' + p.author_name + "</p>" +
      '<div class="card-actions">' +
        '<button class="like-btn ' + (liked ? "liked" : "") + '" data-id="' + p.id + '">' +
          '<i class="' + (liked ? "fa-solid" : "fa-regular") + ' fa-heart"></i> ' + p.likes +
        "</button>" +
        '<button class="download-btn" data-id="' + p.id + '" data-download="' + (p.download_location || "") + '">' +
...
function render(list = g_photos) {
  ...
  for (let i = 0; i < show.length; i++) wrap.appendChild(create_card(show[i]));
}

/* móvil: muestra solo 1 */
function set_mobile_single() {
  const nodes = $all(".photo-card");
  for (let i = 0; i < nodes.length; i++) nodes[i].style.display = (i === 0 ? "block" : "none");
}
...
function mobile_shift(dir) {
  ...
  for (let i = 0; i < nodes.length; i++) {
    if (nodes[i].style.display !== "none") { active = i; break; }
  }
  if (active < 0) active = 0;

  const next = (active + dir + nodes.length) % nodes.length;
  for (let i = 0; i < nodes.length; i++) nodes[i].style.display = (i === next ? "block" : "none");
}
...
function start_auto(resume) {
  ...
  const cont = $("#cardsContainer");
  if (!cont) return;
  if (!should_auto()) return;          // <<< regla simple

  if (!resume) g_translate_x = 0;
...
function stop_auto() {
  if (g_anim_id) cancelAnimationFrame(g_anim_id);
  g_anim_id = null;
}
...
function bind_events() {
  ...
  on(next_btn, "click", () => { if (is_mobile()) mobile_shift(1); });
  on(prev_btn, "click", () => { if (is_mobile()) mobile_shift(-1); });
  ...
  if (wrap) {
    wrap.addEventListener("click", async (e) => {
      ...
      if (dnl) {
        ...
        try { await register_download(loc); } catch {}
        for (let i = 0; i < g_photos.length; i++) {
          if (g_photos[i].id === id && g_photos[i].regular) { window.open(g_photos[i].regular, "_blank"); break; }
        }
      }
    });
  ...
  window.addEventListener("resize", () => {
    if (should_auto()) start_auto(true);
    else stop_auto();
  });
}
  ...
  return {
    async load_highlights() {
      ...
      try {
        ...
        g_photos = r.photos;
        render(g_photos);
        should_auto() ? start_auto(false) : set_mobile_single();   // <<< aquí
      } catch {
    ...
    async from_search(query) {
      try {
        ...
        g_photos = r.photos;
        render(g_photos);
        should_auto() ? start_auto(true) : set_mobile_single();    // <<< y aquí
      } catch {
        /* la galería ya muestra errores */
      }
    },
  };
}
